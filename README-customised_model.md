...
1. **Download the Model**
   ...
   ```bash
   wget "https://drive.google.com/uc?export=download&id=1cE9x3aHxc36TWoJHesJLPYb2lpLSrw7h" -O ALLSorts/models/HongKong_model.zip 
   
   unzip -o ALLSorts/models/HongKong_model.zip -d ALLSorts/models/
   ```
   The extracted `HongKong_model/` directory will contain the following files:

   * `allsorts.pkl.gz`
   * `comparisons/`
   * `comparisons.csv`
   * `cross_val_results.csv`
   * `cross_val_results.csv.gz`
   * `gridsearch/`
...
2. **Run the Customised Model on the Test Data**
   ...
   ```bash
   ALLSorts -model_dir ALLSorts/models/HongKong_model/ \
...
3. **Run the Model on Your Own Data**
   ...
   ```bash
   ALLSorts -model_dir models/HongKong_model/ \
